...
// connection per tab.
async function rebuildFromPages(port, host) {
...
export async function setLabel(tab, label, port, host) {
    ...
    if (label)
        remember(label, tab.id);
    else {
        const prev = idToLabel.get(tab.id);
        if (prev)
            labelToId.delete(prev);
...
export async function resolveLabel(label, port, host) {
